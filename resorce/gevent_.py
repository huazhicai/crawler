...
def get_page_url(_max_page, start=1):
    ...
    while page < _max_page:
        yield ''.join((base_url, 'zufang/pg', str(page)))
        page += 1


def is_root_url(url):
    if re.match(base_url + 'zufang/pg', url, re.S):
        return True
    return False
...
class Crawler:
    def __init__(self, roots,
        ...
        self.roots = roots # 索引页url，生成器
        self.max_tries = max_tries  # 失败后最大尝试次数
        self.max_tasks = max_tasks  # 最大并发数
        self.urls_queue = Queue(loop=self.loop)  # url队列
        self.seen_urls = set()  # url去重集合
        self.session = aiohttp.ClientSession(loop=self.loop)  # 异步会话
        self.href_house = {}  # 字典
        create_csv()
        for root in roots:
            self.urls_queue.put_nowait(root)  # 100 个页面url放入双端队列中

        self.started_at = datetime.now()
        self.end_at = None

    def close(self):
        self.session.close()  # 关闭异步会话
    ...
    async def fetch_etree(response):
        if response.status == 200:
            content_type = response.headers.get('content-type') # 返回内容类型
            if content_type:
                content_type, _ = cgi.parse_header(content_type)
            if content_type in ('text/html', 'application/xml'):
                text = await response.text()   # 可等待对象
                doc = etree.HTML(text)
                return doc
